...
$canOrder	= $user->authorise('core.edit.state', 'com_advertisement');
$saveOrder	= $listOrder == 'a.ordering';
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_advertisement&task=advertisements.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'advertisementList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
...
<?php endif;?>
		...
		<table class="table table-striped" id="advertisementList">
			...
			<?php foreach ($this->items as $i => $item) :
                ...
                <?php if (isset($this->items[0]->state)): ?>
					<td class="center">
						<?php echo JHtml::_('jgrid.published', $item->state, $i, 'advertisements.', $canChange, 'cb'); ?>
					</td>
                <?php endif; ?>
				...
				<?php endif; ?>
				<?php if ($canEdit) : ?>
					<a href="<?php echo JRoute::_('index.php?option=com_advertisement&task=advertisement.edit&id='.(int) $item->id); ?>">
					<?php echo $this->escape($item->advs_name); ?></a>
				<?php else : ?>
					<?php echo $this->escape($item->advs_name); ?>
				<?php endif; ?>
				</td>
				<td>
